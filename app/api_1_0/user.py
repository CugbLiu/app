...
def register():
    ...
    # need modify
    # confirm_url = confirm_ip + url_for('userApi.confirm_email',
    #                                    token=token, _external=False)
    confirm_url = confirm_ip + '/confirm/' +token
...
def forget_password():
    ...
    if email is None:
        ...
        temp = render_template(
            'reset.html', reset_url=reset_url + '?token=' + token, username=u.get('username'))
...
@cross_origin()
@userApi.route('/reset_password', methods=['GET', 'POST'])
def reset_password():
    if request.method == 'POST':
        info = request.values
    else:
    ...
    confirm_password = info.get('confirm_password', None)
        ...
        if email is None:
            ...
            if u == False:
                ...
                try:
                    ...
                    if db.reset_user_password(email, password):
                        # return redirect(login_url)
                        return jsonify({'result': 'success', 'message': "Rest password success"})
                    else:
...
@cross_origin()
@userApi.route('/check_email', methods=['GET', 'POST'])
def check_email():
    if request.method == 'POST':
        info = request.values
    else:
    ...
    if email is not None:
        u = db.query_by_email(email)
        if u is not None:
            return jsonify({'result':'success','email':'yes'})
        else:
            return jsonify({'result': 'success', 'email': 'no'})
    else:
        return jsonify({'result': 'error', 'message': 'the email is None..'})
